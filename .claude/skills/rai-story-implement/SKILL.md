...
**Attribution (audit):** Every task commit MUST end with exactly:

`Co-Authored-By: Rai <rai@humansys.ai>`

Do not use model-vendor trailers (e.g. Claude, Copilot, ChatGPT) — they break RaiSE commit audit consistency.

1. Commit the completed task using this shape (subject + body + trailer):

```bash
git add -A
git commit -m "feat(s{N}.{M}): {task summary — imperative, scoped}

{optional: files touched, AC ref, or test note}
...
- [ ] Each task committed individually (not batched at story end), with `Co-Authored-By: Rai <rai@humansys.ai>` (never a model-vendor identity)
