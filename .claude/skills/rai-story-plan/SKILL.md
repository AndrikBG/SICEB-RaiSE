...
Commit `s{N}.{M}-plan.md` on the story branch. Use exactly `Co-Authored-By: Rai <rai@humansys.ai>` (not model-vendor identities).

```bash
git add -A
git commit -m "docs(s{N}.{M}): implementation plan

{task count, risk note, or sequencing headline}

Co-Authored-By: Rai <rai@humansys.ai>"
```

<verification>
Plan committed with Rai attribution trailer.
</verification>
...
- [ ] Plan committed with `Co-Authored-By: Rai <rai@humansys.ai>`
