...
### Step 6: Commit design artifacts

After writing the YAML artifact and `s{N}.{M}-design.md`, commit on the story branch. Use exactly `Co-Authored-By: Rai <rai@humansys.ai>` (not model-vendor identities).

```bash
git add -A
git commit -m "docs(s{N}.{M}): story design

{1-2 lines: approach summary or key decisions}

Co-Authored-By: Rai <rai@humansys.ai>"
```

<verification>
Design files committed with Rai attribution trailer.
</verification>
...
- [ ] Design artifacts committed with `Co-Authored-By: Rai <rai@humansys.ai>`
