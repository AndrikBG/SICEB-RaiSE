...
export function ServiceInventoryView() {
  const activeBranch = useAuthStore((s) => s.activeBranch);
  const user = useAuthStore((s) => s.user);
  // Service managers have a staffId that maps to their assigned service
  const serviceId = user?.staffId;
  ...
  const loadItems = useCallback(async (pageNum: number) => {
    if (!serviceId) {
      setError('No se encontró servicio asignado');
      setLoading(false);
      return;
    }
    setLoading(true);
  ...
  // Real-time WebSocket updates (only for items in this service)
  useEffect(() => {
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-xl font-bold text-gray-900">Inventario de Mi Servicio</h1>
